...
const JobBlock: FC<JobBlockProps> = ({
  ...
  return (
    <figure className="flex flex-row items-center w-[95%] max-w-[1400px] h-full xl:bg-[white] bg-[#EFF0F5] px-4 py-6 mb-2 rounded-lg relative"
    onClick={()=>navigate(`/${id}`)}
    style={{
      boxShadow: "0px 1px 3px 0px rgba(0, 0, 0, 0.12)"
    }}
    >
      <LazyLoadImage className="aspect-square rounded-full min-w-[66px]" wrapperClassName="mr-[26px]" width={85} src={pictrure} effect="blur" alt="" />
      <div className="xl:my-0 my-6 w-3/4">
        <h3 className="font-bold text-base w-full xl:block hidden"  
        >{title}</h3>
        <h3 className="text-base w-full xl:hidden block"  
        >{title.split(" ", 8).join(" ") + "..."}</h3>
